          ...
          <div class="card">
            <div class="card-body">
              ...
              <form method="POST" action="{{route('displaysolution.store')}}" enctype="multipart/form-data"> 
                {{ csrf_field() }}
              <h5 class="card-title">{{isset($displaysolution->title) ? $displaysolution->title : 'New Display Solution'  }}</h5>
                
                <div class="row mb-3">
                  <label for="inputText" class="col-md-2 col-form-label">Select Solution</label>
                  <div class="col-md-10">
                    <select class="form-select" name="menu_id">
                      ...
                      <option value="{{$value->id}}"
                        @if(!empty($value->id))
                          {{($value->id == $value->id) ? 'selected="selected"' : '' }}
                        @endif/> {{$value->title}}
                      </option>
                      @endforeach
                </select>
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-md-10">                    
                   <input type="text" name="meta_title" class="form-control" value="{{old('meta_title',isset($displaysolution->meta_title) ? $displaysolution->meta_title : '')  }}">                    
                  </div>
                </div>
                <div class="row mb-3">
                  <label for="inputText" class="col-md-2 col-form-label">Meta keywords</label>
                  <div class="col-md-10">                    
                   <input type="text" name="meta_keywords" class="form-control" value="{{old('meta_keywords',isset($displaysolution->meta_keywords) ? $displaysolution->meta_keywords : '')  }}">
                 </div>
                </div>
                <div class="row mb-3">
              ...
              <div class="col-sm-10">
                <textarea name="short_description" class="form-control {{$errors->has('short_description') ? 'border-danger' : '' }} " id="">{{old('short_description',isset($displaysolution->short_description) ? $displaysolution->short_description : '' )}}</textarea>
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-sm-4">
                    <div class="card ">
                        ...
                        <img src="{{asset(isset($displaysolution->image) ? 'images/'.$displaysolution->image : 'images/inds1.jpg')}}" class="card-img-top" alt="..." id="bannerimagepreview">
